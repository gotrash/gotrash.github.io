<template>
  <b-container class="p-3" fluid>
    <admin-page-header-row slug="ADMIN_COUNTRIES" />
    ...
    <b-row>
      <b-col cols="12">
        <admin-filter-card
          ...
          :sort-desc-options="sortDescOptions"
        />
      </b-col>
    </b-row>
    <b-row>
      <b-col cols="12">
        <b-card bodyBgVariant="white" headerBgVariant="white" footerBgVariant="white" no-body>
          <b-card-header class="align-items-center d-flex justify-content-between">
            <b-card-title class="my-1">Countries</b-card-title>
          ...
          <b-card-body class="p-0">
            <b-table :items="countries" :fields="fields"></b-table>
          </b-card-body>
        </b-card>
      </b-col>
    </b-row>
...
<script setup>
  ...
  const _processResponse = async res => {
    countries.value = res.data.value.content;
