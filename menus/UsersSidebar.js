export default [
  { icon: "users", route: "users", text: "NAV__USERS_DASHBOARD" },
  { icon: "unlock", route: "users-password-reset-tokens", text: "NAV__PASSWORD_RESET_TOKENS" },
  { icon: "lock", route: "users-privileges", text: "NAV__PRIVILEGES" },
  { icon: "lock", route: "users-privilege-types", text: "NAV__PRIVILEGE_TYPES" },
  { icon: "users", route: "users-roles", text: "NAV__ROLES" },
  { icon: "users", route: "users-users", text: "NAV__USERS" },
  { icon: "map-marker", route: "users-user-locations", text: "NAV__USER_LOCATIONS" },
  { icon: "user-check", route: "users-verification-tokens", text: "NAV__VERIFICATION_TOKENS" }
];
