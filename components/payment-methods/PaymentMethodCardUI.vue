        ...
        <b-form class="mb-3" @submit.stop.prevent="onUpdateDescription">
          <b-form-group class="text-center text-md-end">
            ...
            <b-form-input
              ...
              @blur="onFieldBlur"
            />
            <small v-else :style="{ 'word-wrap': description.value ? 'break-word' : 'normal' }" @click="onFieldClick">
    ...
    <b-row>
      <b-col cols="12" sm="6">
        <dl class="text-center text-sm-start">
          ...
          <dd>
            <fa-icon
              :icon="paymentMethod.card.checks.addressLine1Check ? 'check' : 'times'"
              :class="[
                'fa-lg fa-fw ms-auto me-auto me-sm-2 me-md-3 d-block d-sm-inline',
                paymentMethod.card.checks.addressLine1Check ? 'text-success' : 'text-danger'
              ]"
            />
            <strong class="mb-1 d-block text-center text-sm-start d-sm-inline">Address Line One</strong>
            <br class="d-none d-sm-block" />
            <fa-icon
              :icon="paymentMethod.card.checks.addressPostcalCodeCheck ? 'check' : 'times'"
              :class="[
                'fa-lg fa-fw ms-auto me-auto me-sm-2 me-md-3 d-block d-sm-inline',
                !paymentMethod.card.checks.addressPostalCodeCheck ? 'text-success' : 'text-danger'
              ]"
            />
            <strong class="mb-1 d-block text-center text-sm-start d-sm-inline">Postcode</strong>
            <br class="d-none d-sm-block" />
            <fa-icon
              :icon="paymentMethod.card.checks.cvcCheck ? 'check' : 'times'"
              :class="[
                'fa-lg fa-fw ms-auto me-auto me-sm-2 me-md-3 d-block d-sm-inline',
                paymentMethod.card.checks.cvcCheck ? 'text-success' : 'text-danger'
              ]"
            />
      ...
      <b-col cols="12" sm="6">
        <dl class="text-center text-sm-end">
          ...
          <dd>
            <address v-html="addressFormatter(paymentMethod.billingDetails.address)"></address>
          </dd>
          <dt></dt>
          <dd>
            <b-button-group>
              <b-button class="text-danger" @click="() => onDelete(paymentMethod.id)">
